...
// =========================================================
(function() {
    const projects = ["museum", "golaab", "tomaan", "promptlab-carbon", "smart-substitutions" , "transparent-jar", "ux-ui-redesign"];
    ...
    else if (currentUrl.includes("ux-ui-redesign")) currentIndex = 6;
...
(function($) {
	...
	// Menu.
		...
		// Carousel dragging and arrow navigation
		var sliders = document.querySelectorAll('.carousel-grid');
		...
		sliders.forEach(function(slider) {
			...
			// Arrow setup if more than 2 tiles
			if (slider.children.length > 2) {
				...
				var btnStyle = "position: absolute; top: 50%; transform: translateY(-50%); background: #1D1D1F; color: #fff; border: none; border-radius: 50%; width: 40px; height: 40px; cursor: pointer; z-index: 10; opacity: 0.8; font-size: 18px; display: flex; align-items: center; justify-content: center;";
				
				prevBtn.style.cssText = btnStyle + "left: -15px;";
				nextBtn.style.cssText = btnStyle + "right: -15px;";

				prevBtn.onmouseover = function() { prevBtn.style.opacity = '1'; };
				prevBtn.onmouseout = function() { prevBtn.style.opacity = '0.8'; };
				nextBtn.onmouseover = function() { nextBtn.style.opacity = '1'; };
				nextBtn.onmouseout = function() { nextBtn.style.opacity = '0.8'; };
				...
				// Small delay to make sure the layout is ready
